...
## 📍 İçindekiler
1. (#aşama-1-planlama--yönlendirme-strateji-ve-güvenlik)
    *  (#-opsec-güvenliği-teknik)
    *  (#️-etik-sınırlar-ve-yasal-çerçeve)
2. (#aşama-2-bilgi-toplama-ham-veri-edinme)
    *  (#-arama-ve-sızıntı-analizi)
    *  (#️-sosyal-medya-istihbaratı-socmint)
    *  (#-teknik-altyapı-taraması)
    *  (#-alan-adı-ve-dns-geçmişi)
    *  (#-kurumsal-istihbaratı-businesscorporate-int)
    *  (#️-resmi-kayıt-istihbaratı-public-records-int)
    *  (#-web-arşivleme)
    *  (#️-görüntü-tersine-arama-imint)
    *  (#-coğrafi-veri-toplama-geoint)
    *  (#️-havacılık-istihbaratı-avint)
    *  (#-marint-denizcilik-istihbaratı)
    *  (#-finint-finansal-istihbarat--kripto-osint)
    *  (#-sigint-sinyal-istihbaratı---halka-açık-yönü)
3. (#aşama-3-️-işleme--değerlendirme-veriyi-ayıklama)
4. (#aşama-4-analiz--üretim-noktaları-birleştirme)
...
5. (#aşama-5--yayim-raporlama)
6. (#-önerilen-kitaplar-ve-ileri-okumalar)
7. (#-akademik-vaka-analizi-operasyon-gölge-ağ)
...
Elbette. Rehberinizi inceledim ve GitHub'da en iyi şekilde görüntülenmesi için Markdown formatını düzelttim. Tutarlılığı artırmak, okunabilirliği iyileştirmek ve tüm bağlantıların/kod bloklarının doğru çalışmasını sağlamak için bazı küçük temizlemeler ve biçimlendirme ayarlamaları yaptım.

İşte son hali:

\<img width="1024" height="765" alt="image" src="[https://github.com/user-attachments/assets/17c82956-1ce5-49fe-9125-9a6641271ddc](https://github.com/user-attachments/assets/17c82956-1ce5-49fe-9125-9a6641271ddc)" /\>
...
-   **Sahte Profiller:** Basit konular için sahte profiller oluşturmak için `temp-mail` gibi geçici e-posta servisleri. Ama önemli konularda dikkat çekmemek ve doğal akışta bir istihbarat toplamak için ya kendi sunucunuza bağlı bir mail ya da `Proton` gibi üst seviye güvenlikli mail servisleri kullanın!

-   Yapay zeka ile üretilmiş, gerçek hayatta olmayan kişi fotoğrafı üreten bir site:(https://thispersondoesnotexist.com/). Sahte profillerinizde kullanabilirsiniz.
...
-   **Teknik İlişkilendirmeyi (Attribution) Kırmak:** Akıllı hedefler sizi sadece IP'nizden tanımaz. Tarayıcıdaki parmak izinize (`Browser Fingerprint`) odaklanırlar: Kullandığınız yazı tipleri (fonts), ekran çözünürlüğünüz, tarayıcı eklentileriniz... Milyonlarca insan arasında sizi "benzersiz" yapan bir imzadır bu.
    ...
    -   **EK OLARAK:**
        -   Tarayıcılarınızın kendine has özel gizlilik ayarlarını maksimum düzeyde kullanın. Özel ayarlarda var olan (örneğin Firefox'ta `privacy.resistFingerprinting`) özellikleri kullanmak vb.
        -   Güvenilir olmayan kaynaklarda (`.onion` uzantılı siteler gibi) arama yaparken tarayıcınızı orijinal (Resolution) çözünürlüğünüzde kullanmamaya çalışın! Siteler genelde bunları logluyor.
...
-   **Dev Referans GitHub Kaynakları:** Alanında en çok kabul gören ve sürekli güncellenen araç ve kaynak koleksiyonları.
    ...
    -   [github.com/cipher387/osint_stuff_tool_collection](https://github.com/cipher387/osint_stuff_tool_collection)
...
-   **Google Dork ve Meta-Arama Motorları:** Gelişmiş arama operatörleriyle gizli dosya, veritabanı ve sızıntıları bulma.
    -   Örnek kullanım:(https://exposingtheinvisible.org/guides/google-dorking/)
...
-   **Araç Plakaları:** Araç plakalarının hangi ülkeye ait olduğunu tarihsel olarak depolayan bir veritabanı:(http://www.worldlicenseplates.com/).
...
-   **Görsel Netleştirme:** Photoshop bilginiz yoksa, `unblur` veya `deblur` araçlarıyla bulanık fotoğrafları netleştirebilirsiniz.
...
## 🎓 Akademik Vaka Analizi: "Operasyon: Gölge Ağ"

### Senaryo: "Operasyon: Gölge Ağ" (Akademik Vaka Analizi)

**Hedef:** "Ph03nix_7" kod adlı kurgusal bir tehdit aktörünün kimliğini, operasyonel konumunu ve para aklama yöntemlerini, birden fazla istihbarat disiplinini (IMINT, MARINT, FININT, Teknik INT) çapraz doğrulayarak (cross-corroborate) tespit etmek.
...
-   **IMINT (Meta Veri Analizi):** Fotoğraf **`ExifTool`** ile analiz edilir:
    ```
    exiftool Ph03nix_tweet.jpg
    ```
